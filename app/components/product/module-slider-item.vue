<template>
  <ClientOnly>
    <swiper-container :pagination="{ clickable: true, currentClass: 'bg-primary' }" :autoplay="true" style="z-index: 0"
      ref="containerRef">
      <swiper-slide v-for="(item, index) in listProduct?.getData" :key="index" class="p-2">
        <div class="card border-0 h-100 shadow-sm mb-5">
          <div class="ratio ratio-1x1">
            <img :src="item.image_urls?.[0]?.url || '/images/image-error.svg'" class="card-img-top" alt="..." />
          </div>
          <div class="card-body">
            <small class="text-primary fst-italic">
              Giá bán : {{ formatCurrency(item.gia_nt2) }}</small>
            <NuxtLink :to="`/product/${item.ma_vt}`" class="text-decoration-none text-dark">
              <small class="text-truncate-2">
                {{ item.ten_vt }}
              </small>
            </NuxtLink>
          </div>
          <nuxt-link :to="`/product/${item.ma_vt}`"
  ...
  <template v-if="(listProduct?.getData?.length || 0) === 0">
    <div class="text-center py-5">
      <UiEmpty />
      <h5 class="text-muted">Không có sản phẩm nổi bật nào.</h5>
    </div>
  </template>
</template>
