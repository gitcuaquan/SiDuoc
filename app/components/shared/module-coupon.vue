<template>
  <div v-if="coupon" role="button" :class="[
    ...
    <div class="bg-primary flex-grow-1 p-2 rounded-1 bg-opacity-10 position-relative">
      <div class="position-absolute top-0 end-0 me-1 mt-1">
        <input type="checkbox" class="form-check-input" v-model="coupon.isValid" />
      </div>
      <div style="font-size: 14px;" class="m-0 fw-bold pe-2">{{ coupon.discountName }}</div>
      ...
      <div class="d-flex justify-content-between align-items-center">
        <template v-if="coupon?.discountType?.toUpperCase() === 'M'">
          <div v-if="coupon.totalDiscount" style="font-size: 11px" class="text-secondary">
            Giảm {{ formatCurrency(coupon.totalDiscount || 0) }} cho tổng giá trị đơn hàng
          </div>
        </template>
        ...
        <template v-if="coupon?.discountType?.toUpperCase() === 'H'">
          <div v-if="!coupon.quantityGift && coupon.itemNameGift" style="font-size: 11px" class="text-secondary">
        ...
        <div v-if="coupon.maxDiscount" style="font-size: 11px" class="text-secondary">
          ( Giảm tối đa {{ formatCurrency(coupon.maxDiscount || 0) }})
        </div>
      </div>
    </div>
  </div>
