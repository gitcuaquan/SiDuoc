...
<template>
  <ClientOnly>
    <swiper-container :coverflowEffect="{
      rotate: 50,
      stretch: 1,
      depth: 120,
      modifier: 1,
    }" ref="containerRef" style="z-index: 0 !important; padding-top: 50px; padding-bottom: 50px" :init="false">
      <swiper-slide v-for="(slide, idx) in feedbacks" :key="idx" class="my-3">
        <div class="card border-2 rounded overflow-hidden border-primary border-opacity-25 shadow-sm">
          <div class="card-body" style="background: #ffffff !important">
            <div class="d-flex mb-2 gap-6 justify-content-end">
              <Star v-for="value in 5" :size="16" fill="#ffc107" style="color: #ffc107" />
            </div>
            <p class="card-text">
