<template>
  ...
  <ClientOnly>
    <AuthModalProductSuggest
      v-if="showModalDetail"
      @created="getProductSuggest"
      @close="showModalDetail = null"
    />
  </ClientOnly>
</template>
...
function deleteProduct(product: ITemsTapmed) {
  if(confirm("Bạn có chắc chắn xoá sản phẩm đề xuất này không?"))
  $appServices.items.addProductSuggest({
    isDelete: true,
    ma_vt: product.ma_vt,
    list_code:"HOME"
  }).then(() => {
    useToast().success("Xoá sản phẩm đề xuất thành công");
    getProductSuggest();
  }).catch((error) => {
    useToast().error("Xoá sản phẩm đề xuất thất bại");
