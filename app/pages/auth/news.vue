<template>
  <LayoutAuth :loading="loading" :breadcrumb="breadcrumb" :is-empty="listNews?.data?.length === 0"
    ...
    <div v-if="listNews" class="row g-3">
      <div class="col-lg-8 col-12">
        <input type="search" class="form-control" placeholder="Tìm kiếm..." v-model="keyword" />
      </div>
      <div class="col-lg-4">
        <select class="form-select" v-model="queryList.category" @change="fetchNewsList">
      ...
      <div class="col-12">
        <div class="cart-header table-fake d-none border w-100 d-lg-table-row bg-light fw-semibold">
          ...
          <span class="d-lg-table-cell text-nowrap p-3 py-2 text-end text-nowrap">
            Ngày tạo
          </span>
          <span class="d-lg-table-cell text-nowrap p-3 py-2 text-end text-nowrap">
            Hành động
        ...
        <!-- Item -->
        <div v-for="value in listNews.data"
          ...
          <div
            ...
            <div class="d-flex flex-column align-items-center justify-content-center">
              <div class="ratio ratio-1x1" style="width: 80px; height: 80px">
                <img :src="value.thumbnail || '/images/image-error.svg'" alt="thumbnail" class="img-thumbnail" />
              </div>
            </div>
          </div>
          ...
          <div
            ...
            <div class="d-flex flex-column align-items-end align-items-lg-start justify-content-center">
          ...
          <div
            ...
            <div class="d-flex justify-content-end gap-2">
              <button @click="editNews(value._id)" class="btn btn-sm btn-primary">
                Sửa
              </button>
              <button @click="deleteNews(value._id)" class="btn btn-sm btn-danger">
                Xóa
              </button>
            </div>
          </div>
        ...
        <div class="mt-3" v-if="listNews">
          <SharedModulePagination :pagination="listNews.meta" @page-change="pageChanged" />
        </div>
      </div>
    </div>
  </LayoutAuth>
  <ClientOnly>
    <AuthModalActNews :edit-id="editId" v-if="showModal" @close="showModal = false" @created="fetchNewsList" />
  </ClientOnly>
</template>
