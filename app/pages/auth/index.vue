<template>
  <LayoutAuth :loading="loading" :name="'Đơn hàng của bạn'" :breadcrumb="breadcrumb">
    <template v-if="listOrder?.getData?.length">
      <input type="search" v-model="keywordSearch" placeholder="Tìm kiếm đơn hàng" class="form-control mb-2" />
      <div class="cart-header d-none border w-100 d-lg-table-row bg-light fw-semibold">
        ...
        <span class="d-lg-table-cell p-3 py-2 text-center text-nowrap">Hành Động</span>
      </div>

      <!-- Item -->
      <div v-for="(order, index) in listOrder?.getData"
        ...
        <div
          ...
          <div class="d-flex justify-content-end gap-1">
            <button @click="showModalDetail = order.stt_rec" class="btn-sm text-nowrap me-lg-2 btn btn-primary">
              <Eye :size="16" />
            </button>
            <button @click="showModalCreateTicket = true" class="btn-sm text-nowrap me-lg-2 btn btn-danger">
      ...
      <div class="mt-3">
        <SharedModulePagination :pagination="listOrder?.pagination" @page-change="
  ...
  <ClientOnly>
    <AuthModalOrderDetail v-if="showModalDetail" @close="showModalDetail = undefined" :order_id="showModalDetail"
      @ticket="
        showModalCreateTicket = true;
      showModalDetail = undefined;
      " />
    <AuthModalCreateTicket v-if="showModalCreateTicket" @close="showModalCreateTicket = false" />
  </ClientOnly>
</template>
